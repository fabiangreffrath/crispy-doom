...
// bot cursor (activates botreadout when facing bot)
int viewingbot;
int lastbot;
int readoutcolor;
...
#define TITLESCREEN_X_OFFSET 30
...
// moved here from marshmallow.h
#define MENU_WALLPAPER "COMP03_1"
#define INFO_OUTPUT_X 160 // seems to be used by both botreadout and inforeadout... must separate these
#define INFOREADOUT_Y_OFFSET 100


// IDEA: make these on the enums for each menu:  typedef enum invmenu_t; invmenu_t invmenu_selection;
...
enum {
	...
	SKIP_SELECTED, // now used for song title
...
enum {
	...
	//ITEMRESPAWN_SELECTED, // show only if (netgame)
...
enum {
	...
	KILL_BOTS_SELECTED,
	// add LINE BREAK here
	//BOT_STATES_SELECTED,   // this is no longer needed; maybe make this an option to show either state or weapon, where state is more for debugging purposes
	// add whatever else we need on this line
	// already a line break here
	BOT1_WEAPON,
	BOT2_WEAPON,
	BOT3_WEAPON,
	...
	BOTSPEED_SELECTED, // new
	
	BOTSTATES_SELECTED, // new
...
enum {
	...
	PROFILE_SELECTED,
	OPTIONS_SELECTED,
	BOTMENU_SELECTED, // was SKIPMAP_SELECTED,
	MUSIC_SELECTED,
	SKIPMAP_SELECTED, // was BOTMENU_SELECTED,
	SANDBOX_SELECTED, // was CHANGESKILL_SELECTED,
	MESSAGES_SELECTED,
	MAX_MAINMENU_ITEMS,  // TEMP moved here
...
hu_stext_t profile_bosses_killed;  // count every spiderdemon and cyberdemon?  what about barons only if killed on E1M8?
hu_stext_t profile_secrets_found;  // do we keep counting the same secrets over and over?  probably yeah
hu_stext_t profile_sandbox_kills;
hu_stext_t profile_sandbox_games_won;
hu_stext_t profile_sandbox_bosses_killed;
hu_stext_t profile_levels_completed;  // only if exit switch is hit organically with no cheats
hu_stext_t profile_episodes_completed;  // only counts if all 8 maps were completed organically with no cheats
hu_stext_t profile_games_completed; // only when an entire iwad is completed organically
...
// profile macros so it's easier to rearrange lines
...
//#define MAINMENU_LINE7  mainmenu_changeskill
#define MAINMENU_LINE8	mainmenu_sandbox       // first selectable one if (netgame && !solonetgame)  (set solonetgame on reading -solo-net, and when we enable/disable bots)
#define MAINMENU_LINE9	mainmenu_messages

#define MAINMENU_LINE10 mainmenu_suicide  // removed this line for now


// macros for every HUD text line
